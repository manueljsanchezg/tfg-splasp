...
async def test_analyze_batch_snap_project(async_client: AsyncClient, sample_xml_bytes: bytes):
    token = await _get_auth_token(async_client)
    headers = {"Authorization": f"Bearer {token}"}
...
async def test_get_project_analysis_by_versions_ids(async_client: AsyncClient):
    token = await _get_auth_token(async_client)
    headers = {"Authorization": f"Bearer {token}"}
...
async def test_get_version_analysis_not_found(async_client: AsyncClient):
    token = await _get_auth_token(async_client)
    headers = {"Authorization": f"Bearer {token}"}
...
async def test_analyze_snap_project_no_file_url(async_client: AsyncClient):
    res = await async_client.post("/api/analyses")
    assert res.status_code == 400


async def test_analyze_snap_project_anonymous_no_file_url(async_client: AsyncClient):
    token = await _get_auth_token(async_client)
    headers = {"Authorization": f"Bearer {token}"}
...
async def test_analyze_batch_snap_project_no_file_url(async_client: AsyncClient):
    token = await _get_auth_token(async_client)
    headers = {"Authorization": f"Bearer {token}"}
