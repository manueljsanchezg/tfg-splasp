...
import type { ProjectResponse, ProjectVersionResponse, SavedAnalysisResult } from '../../types/project'
...
function SessionInfoPage() {
  ...
  const handleOpenModal = async (version: ProjectVersionResponse, projectTitle: string) => {
  ...
  return (
    <div className="flex flex-col gap-6 w-full px-8 py-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <button className="btn btn-outline btn-lg text-xl" onClick={() => navigate('/sessions')}>
          Back to Sessions
        </button>
      </div>
      ...
      <div className="bg-base-100 rounded-xl shadow-lg border border-base-300 overflow-hidden">
        <table className="table table-lg w-full">
          ...
          <tbody>
            {isLoadingProjects ? (
              <tr><td colSpan={2} className="text-center py-12 text-2xl">Loading...</td></tr>
            ) : projects.length === 0 ? (
              <tr><td colSpan={2} className="text-center py-12 text-2xl">No projects found.</td></tr>
            ) : (
      ...
      <dialog className={`modal ${isModalOpen ? 'modal-open' : ''}`}>
        <div className="modal-box w-11/12 max-w-7xl h-[90vh] flex flex-col p-0">
          ...
          <div className="p-8 bg-base-200 flex justify-between items-center border-b border-base-300">
            ...
            <button className="btn btn-ghost btn-lg text-2xl" onClick={() => setIsModalOpen(false)}>✕</button>
          </div>
          ...
          <div className="p-8 overflow-y-auto flex-1 bg-base-100">
            {isLoadingAnalysis ? (
              <div className="flex flex-col items-center justify-center h-full gap-4">
                ...
                <span className="text-2xl font-medium text-base-content/70">Loading analysis...</span>
              </div>
            ) : selectedAnalysis ? (
