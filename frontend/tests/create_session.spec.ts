...
test('Create session and assert it appears', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'e.g., Advanced Programming' }).fill('New session test');
  ...
  await expect(page.getByRole('cell', { name: 'New session test' }).first()).toBeVisible();
});